...
const BASE_URL = `https://my.jasminsoftware.com/api/${process.env.TENANT}/${process.env.ORGANIZATION}/invoiceReceipt/invoices`;

export const getAllInvoices = async () => {
    const token = await getAccessToken();
    const url = `${BASE_URL}/odata?$select=id,currencyDescription,seriesNumber,accountingPartyName,grossValueAmount,totalLiabilityAmount,discountInValueAmountAmount,company,documentType,serie,accountingParty,sellerSupplierParty`;
    try {
        ...
        console.error("Erro ao obter todas as faturas do fornecedor:", error.message);
        throw new Error("Falha ao buscar faturas do fornecedor. Verifique o serviço e a URL.");
    }
};
...
export const getInvoiceReport = async (id) => {
    const token = await getAccessToken();
    const url = `${BASE_URL}/${id}/printOriginal?template=InvoiceReceipt_InvoiceReport`;
...
export const getInvoiceByIDNaturalKey = async (id) => {
    const token = await getAccessToken();
